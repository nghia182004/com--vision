...
def preprocessing():
    ...
    for key in num_classes:
        ...
        class_files_training = class_files[: int(len(class_files)*.80)] # get 80% class images fo training
        class_files_testing = class_files[int(len(class_files)*.10) :] # get 10% class images fo testing
        class_files_validation = class_files[int(len(class_files)*.10) :] # get 10% class images fo validating
        ...
        for f in class_files_validation:
            if "," in f or "#" in f or " " in f:
                tmp_f = f.replace(",", "")
                tmp_f = tmp_f.replace("#", "")
                tmp_f = tmp_f.replace(" ", "_")
                os.rename(IMAGES_PATH+"/"+key+"/"+f, IMAGES_PATH+"/"+key+"/"+tmp_f)
                f = tmp_f
            validate_csv_supp.append(f + ","+str(key))    

    random.shuffle(train_csv_supp)
    random.shuffle(test_csv_supp)
    random.shuffle(validate_csv_supp)
    ...
    test_csv_file = open("validate_file.csv", "w+")
    test_csv_file.write(validate_csv)
    test_csv_file.close()
